# Source Register

| Source | Format | Role | Status | Safe use | Notes |
| --- | --- | --- | --- | --- | --- |
| | | Primary / Supporting / Contextual | Unread / Indexed / Reviewed / Verified for citation | Facts / Framing / Cross-check only | |

## Status rules

- `Unread`: file exists but has not been checked
- `Indexed`: inventoried only
- `Reviewed`: read closely enough to use with care
- `Verified for citation`: safe to cite directly in the body

## Safe use rules

- `Facts`: safe for direct factual claims
- `Framing`: safe for structure, wording direction, or comparison only
- `Cross-check only`: use to test or clarify another source

## Notes

- Record filename mismatches here instead of trusting memory.
- Record when a PDF replaced an older PPTX or document file.
- Record any source that is too weak to carry a claim on its own.
